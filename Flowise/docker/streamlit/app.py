...
# Defaults (sobrescritos por _load_app_config)
CHAT_READY = True
CHAT_CONFIG_ERROR = ""
CHAT_BACKEND = "openai"
FLOWISE_API_URL = ""
...
def _sync_config_from_module(cfg) -> None:
    global CHAT_BACKEND, CHROMA_COLLECTION_NAME, CHROMA_ENABLED, CHROMA_PERSIST_DIRECTORY
    global DUCKDB_DATABASE_PATH, DUCKDB_SOURCE_DIR, FLOWISE_API_TOKEN, FLOWISE_API_URL
    global KNOWLEDGE_TXT_DIR, OPENAI_API_KEY, OPENAI_CHAT_MODEL, OPENAI_EMBEDDING_MODEL
    global REQUEST_CONNECT_TIMEOUT_SECONDS, REQUEST_READ_TIMEOUT_SECONDS, CHAT_PROMPT_PREFIX
    ...
    global GUARDRAILS_RATE_WINDOW_SECONDS, GUARDRAILS_LINK_ALLOWLIST, APP_CONFIG_VERSION

    CHAT_BACKEND = cfg.CHAT_BACKEND
    CHROMA_COLLECTION_NAME = cfg.CHROMA_COLLECTION_NAME
    CHROMA_ENABLED = cfg.CHROMA_ENABLED
    CHROMA_PERSIST_DIRECTORY = cfg.CHROMA_PERSIST_DIRECTORY
    DUCKDB_DATABASE_PATH = cfg.DUCKDB_DATABASE_PATH
    DUCKDB_SOURCE_DIR = cfg.DUCKDB_SOURCE_DIR
    FLOWISE_API_TOKEN = cfg.FLOWISE_API_TOKEN
    FLOWISE_API_URL = cfg.FLOWISE_API_URL
    KNOWLEDGE_TXT_DIR = cfg.KNOWLEDGE_TXT_DIR
    ...
    CHAT_PROMPT_PREFIX = cfg.CHAT_PROMPT_PREFIX
    APP_ENV = cfg.APP_ENV
    GUARDRAILS_ENABLED = cfg.GUARDRAILS_ENABLED
    GUARDRAILS_MAX_INPUT_CHARS = cfg.GUARDRAILS_MAX_INPUT_CHARS
    GUARDRAILS_BLOCK_INJECTION = cfg.GUARDRAILS_BLOCK_INJECTION
    GUARDRAILS_BLOCK_ON_PII = cfg.GUARDRAILS_BLOCK_ON_PII
    GUARDRAILS_APPEND_DISCLAIMER = cfg.GUARDRAILS_APPEND_DISCLAIMER
    GUARDRAILS_RATE_LIMIT = cfg.GUARDRAILS_RATE_LIMIT
    GUARDRAILS_RATE_WINDOW_SECONDS = cfg.GUARDRAILS_RATE_WINDOW_SECONDS
    GUARDRAILS_LINK_ALLOWLIST = cfg.GUARDRAILS_LINK_ALLOWLIST
    APP_CONFIG_VERSION = getattr(cfg, "APP_CONFIG_VERSION", "unknown")


_cfg, CHAT_READY, CHAT_CONFIG_ERROR = _load_app_config()
try:
    _sync_config_from_module(_cfg)
except Exception as exc:
    CHAT_READY = False
...
if not CHAT_READY:
    ...
    st.caption(f"Chat: Flowise (Docker local) — config `{APP_CONFIG_VERSION}`.")
